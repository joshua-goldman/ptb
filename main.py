...
# Model options are: 'small', 'medium', 'large', 'test'
model = 'small'
data_path = 'data'
save_path = 'save'
use_fp16 = False
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        grads, irrelevant = tf.clip_by_global_norm(tf.gradients(self._cost, tvars),
    ...
    def _build_rnn_graph_cudnn(self, inputs, config, is_training):
        ...
        c = tf.zeros(
            [config.num_layers, self.batch_size, config.hidden_size],
            tf.float32
        )
        h = tf.zeros(
            [config.num_layers, self.batch_size, config.hidden_size],
    ...
    def _build_rnn_graph_lstm(self, inputs, config, is_training):
        ...
        self._initial_state = cell.zero_state(config.batch_size, data_type(use_fp16))
...
def run_epoch(session, model, eval_op=None, verbose=False):
    ...
    for step in range(model.input.epoch_size):
        ...
        if verbose and step % (model.input.epoch_size // 10) == 10:
            print("{} perplexity: {} speed: {} wps".format(
                step * 1.0 / model.input.epoch_size, 
                np.exp(costs / iters),
                iters * model.input.batch_size * max(1, num_gpus) / (time.time() - start_time),
                )
            )
...
def main(irrelevant):
    ...
    if num_gpus > len(gpus):
        raise ValueError(
            ...
            "gpus which is less than the requested num_gpus={}.".format(
                (len(gpus), num_gpus)
            )
        )

    raw_data = reader.ptb_raw_data(data_path)
    train_data, valid_data, test_data, irrelevant = raw_data
    ...
    with tf.Graph().as_default():
        ...
        if tf.__version__ < "1.1.0" and num_gpus > 1:
            raise ValueError(
                "num_gpus > 1 is not supported for TensorFlow versions below 1.1.0"
            )
        ...
        with sv.managed_session(config=config_proto) as session:
            for i in range(config.max_max_epoch):
                ...
                train_perplexity = run_epoch(session, m, eval_op=m.train_op,
                                            verbose=True)
                print("Epoch: {} Train Perplexity: {}".format(i + 1, train_perplexity))
                valid_perplexity = run_epoch(session, mvalid)
                print("Epoch: {} Valid Perplexity: {}".format(i + 1, valid_perplexity))
